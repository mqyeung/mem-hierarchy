...
int main (int argc, char* argv[]) // the program runs like this: ./program <filename> <mode>
{
	...
	double L1_miss_rate, L2_miss_rate, VI_miss_rate, AAT; 
	//compute the stats here:
	//cout << "\nreturnMissL1()" << myCache.returnMissL1();
	//cout << "\nreturnAccL1()" << myCache.returnAccL1();
	L1_miss_rate = myCache.returnMissL1()/(myCache.returnAccL1() + myCache.returnMissL1());
	L2_miss_rate = myCache.returnMissL2()/(myCache.returnAccL2() + myCache.returnMissL2());
	VI_miss_rate = myCache.returnMissVI()/(myCache.returnAccVI() + myCache.returnMissVI());
	AAT = 1 + L1_miss_rate * (1 + VI_miss_rate * (8 + L2_miss_rate*100));
